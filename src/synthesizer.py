...
2. A numbered SOURCE LIST is provided below the tool results, built directly
   ...
   (b) RATIONALE/reasoning ("why" questions) genuinely requires a citation
       ...
       CRITICAL, real failure mode found via direct testing -- "clear,
       direct answer" for a why-question means the cited source explicitly
       STATES A REASON: it explains why a choice was made, what problem it
       solved, what alternative it replaced, or what tradeoff motivated it.
       It is NOT enough that a source merely DESCRIBES THE CURRENT BEHAVIOR
       or is topically adjacent to the question. A source can be genuinely
       real, genuinely retrieved, and genuinely about the same feature, and
       still fail this bar -- e.g. a PR titled "add original error property"
       that resolves a DIFFERENT, unrelated issue number describes what was
       changed, not why an earlier, unrelated design decision was made; a
       doc section explaining a general language/framework convention (e.g.
       "leading underscores mean private" as a general Python idiom)
       explains the convention, not this specific codebase's own reason for
       adopting it. Before citing any source for a why-question, ask
       yourself: does this source contain an actual justification,
       explanation of a problem being solved, or comparison to an
       alternative -- in its own words? If it only describes the resulting
       behavior, what changed, or a generic convention, without ever
       stating a REASON, that is NOT sufficient evidence -- abstain per
       rule 3 instead, even if a topically-relevant source_id exists in the
       list. Plausible inference ("this was probably done for consistency")
       is exactly what abstaining protects against -- do not let a real,
       relevant-looking citation substitute for a real, stated reason.
